...
Single source of truth for how claude-pal authenticates.
...
```
/pal-setup     # creates the `claude-pal-claude` named volume + `claude-pal-workspace` container
/pal-login     # browser flow, run once per workspace lifetime
```

Your shell only needs `GH_TOKEN`. There is **no** `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` env-var path in claude-pal.

## Why not env-var auth

Earlier drafts of claude-pal used env-passthrough — the host shell held a `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`), and every run forwarded it to an ephemeral container via `docker run -e`. The workspace-container rework removed that path entirely. The rationale (summarized from [`docs/superpowers/specs/2026-04-21-claude-pal-auth-rework.md`](superpowers/specs/2026-04-21-claude-pal-auth-rework.md) §4):
...
- **Credentials never touch the host filesystem.** `.credentials.json` lives in the `claude-pal-claude` Docker volume; it never appears in `~/.bashrc`, `~/.zshrc`, or `~/.claude/` on the host.
...
Inside the workspace, at `/home/agent/.claude/.credentials.json`. On the host, that's backed by the `claude-pal-claude` named Docker volume.
...
- Do not deploy claude-pal as a shared service for a team — for commercial / multi-user scenarios, use a Console account (and still log in via `/pal-login` inside your own workspace on your own machine), or use `claude-pal-action` on a shared runner.

## Migration from env-var auth

If you installed an earlier version of claude-pal, you likely have these in your shell profile:
...
Both are unused by the current claude-pal. To migrate:
...
| `docker volume inspect claude-pal-claude` errors with "no such volume" | The named volume was deleted (manually, or by `docker system prune --volumes`) | `/pal-workspace start` recreates the volume; then `/pal-login` to mint credentials again |
...
- [claude-pal auth-rework design spec](superpowers/specs/2026-04-21-claude-pal-auth-rework.md)
